...
seqstor::seqstor(ftpsocket* ptranSOCKET, ftpsocket* pSOCKET)
{
	int                 rc = RET_SUCCESS;
	// new. delete
	ecode*              pec= (ecode*)0;
...
/*!
 * @file		seqstor.cpp
 * @fn			int seqstor::initialize(void)
 * @brief		メンバ変数初期化
 * @details		デフォルト値設定
 ...
 */
int seqstor::initialize(void)
{
    this->variables();
    this->mmyState = ESTATE::eUPLANS;                   // SND/ANSWER
    return RET_SUCCESS;
}

/*!
 * @file		seqstor.cpp
 * @fn			int seqstor::variables(void)
 * @brief		メンバ変数初期化
 ...
 * @note		initialize()は制御系の初期化
 */
int seqstor::enter(void* pArgs)
{
	int                 rc = (int)this->mmyState;
    std::string         str_dir = sharedObj.mstrSrvDir[EPRM_SRVDIR::eFEEDBACK];
    std::string         str_cmd = "";
    std::string         str_rsp = "";
    // new, delete
    ...
    pcwd = new ftpcmd(mpSOCKET, str_cmd.c_str()/*"CWD SND/ANSWER\x0d\x0a"*/);
	if (pcwd == (ftpcmd*)0) {
     ...
     */
    LOGTRACE(ECTRL::eWH, ERANK::eDBG, ENAME::eSTOR, "CWD SND/ANSWER\n");
...
int seqstor::exec(void* pArgs)
{
     ...
     */
    sharedObj.sync_wait();
    sharedObj.mOrder.getfn(EFOLDER::eANS, mstrPathname);
    sharedObj.sync_post();
    ...
    hthr = ::create_thr(ETHRNAME::eSTOR_ANS, mpSOCKET[ESOCK::eTRAN]);
    ...
    // backgroundでも、送信を行う。forgroundが先に送信しないとダメ。
    // accept()前にコマンド送信。
    // デバッガで、1Stepずつ実行すると、group2()はエラーを返す。
    LOGTRACE(ECTRL::eWH, ERANK::eDBG, ENAME::eSTOR, "%s\n", str_cmd.c_str());
                                                        // DBGCOUT("STOR [filename]\n");
...
/*!
 * @file          seqstor.cpp
 * @fn            int seqstor::exit(void* pArgs)
 * @brief         シーケンスあと処理
 * @details       TBD
